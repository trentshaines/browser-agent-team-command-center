...
FILE_STORE_DIR = Path(__file__).parent / "file_store"
...
def _read_file_content(path: Path) -> str | None:
	"""Read a file's content as text. Uses vision for PDFs/images."""
...
def _extract_pdf_text(path: Path) -> str | None:
	"""Extract text from a PDF using Bedrock vision on each page's bytes."""
	try:
		raw = path.read_bytes()
...
def _extract_image_text(path: Path) -> str | None:
	"""Describe an image file using Bedrock vision."""
	try:
		raw = path.read_bytes()
		suffix = path.suffix.lower()
...
def check_file_store(handoff_reason: str) -> str | None:
	"""Check if file_store contains files that can resolve a handoff.
	...
	files = [f for f in FILE_STORE_DIR.iterdir() if f.is_file() and not f.name.startswith(".")]
	if not files:
		return None

	# Build inventory
	inventory = "\n".join(f"- {f.name} ({f.suffix}, {f.stat().st_size} bytes)" for f in files)
